 ...
 */
public abstract class UploadDrugImageModel extends AbsICmd {
    private BasicModel basicModel = new BasicModel();

    private File file;

    protected UploadDrugImageModel(File file) {
        this.file = file;
    ...
    protected void invoke() {
        String url = basicModel.BASE_API + basicModel.UPLOAD_DRUG_IMAGE;
        ...
        basicModel.requestServer.uploadImage(url, session, file, new ResponseHandle<RESP_Upload_Image>(RESP_Upload_Image.class) {
            ...
            protected void onSuccess(RESP_Upload_Image obj) {
                UploadDrugImageModel.this.onSuccess(obj);
            }

            @Override
            protected void onError(Error error) {
                UploadDrugImageModel.this.onError(error.getCode());
            }
        });
    }

...
public class EditShareActivityPresenter extends BasicPresenter {

    private IEditShareView view;

    public EditShareActivityPresenter(IEditShareView basicView) {
        super(basicView);
        view = basicView;
    }
                                ...
                                new SaveObjectModel<Mbr>(object) {
                                    ...
                                    protected void onSuccess() {
                                        if (view.getActivity() != null) {
                                            ...
                                            intent.putExtra(Constants.MRB_ID, mrbId);
    ...
    public void deleteShare(int id_share, int mrbId, int shareType){
        if (!NetworkUtils.isConnected(view.getActivity())) {
            view.showLongToast("Vui lòng kiểm tra kết nối mạng và thử lại.");
            return;
        }
        DeleteShare deleteShare = new DeleteShare(id_share, shareType);
        iCmd(2, deleteShare, mrbId);
    }
}
